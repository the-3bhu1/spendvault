...
export default function Transactions() {
  const { data, pendingTransfer, setPendingTransfer, smsQueue, removeFromSmsQueue, addTransaction, updateTransaction, reorderTransactions, deleteTransaction } = useFinance();
  ...
  const handleSave = () => {
    ...
    if (newTx.rewardEarnedType === 'delayed' && !finalRewardEarned) {
      if ((account?.type === 'credit_card' || account?.type === 'debit_card') && newTx.type === 'debit' && !newTx.isTravelTransaction && finalCategory !== 'Transfer' && finalCategory !== 'CC Payment') {
  ...
  const filteredIncome = filteredTransactions.reduce((sum, tx) => {
    const isExcludedCategory = tx.category.toLowerCase() === 'transfer' || tx.category.toLowerCase() === 'cc payment';
                    ...
                    <div className="fade-in">
                      {Object.entries(groupedByDate).sort((a, b) => new Date(b[0]).getTime() - new Date(a[0]).getTime()).map(([date, txs]) => {
                        ...
                        const dailyIncome = txs.reduce((sum, t) => {
                          const isExcludedCategory = t.category.toLowerCase() === 'transfer' || t.category.toLowerCase() === 'cc payment';
                          if (isExcludedCategory) return sum;
            ...
            <div className="modal-body">
              ...
              <div className="grid grid-cols-2 gap-4" style={{ marginBottom: '1rem' }}>
                ...
                <CustomPicker
                  ...
                  onChange={val => setNewTx({ ...newTx, type: val as TransactionType })}
                ...
                <div className="input-group">
                  ...
                  <div className="grid grid-cols-2 gap-2" style={{ marginTop: '0.25rem' }}>
                    <button
                      ...
                      onClick={() => setNewTx({ ...newTx, isTravelTransaction: false })}
                    >
                      💳 Payments
                    </button>
                    <button
                      className={`btn ${!!newTx.isTravelTransaction ? 'btn-primary' : 'btn-secondary'}`}
                      style={{ padding: '0.75rem', borderRadius: '12px' }}
                      onClick={() => setNewTx({ ...newTx, isTravelTransaction: true })}
                    >
                      🚇 Travel
                    </button>
                  </div>
              ...
              {newTx.category?.toLowerCase() !== 'transfer' && newTx.category?.toLowerCase() !== 'cc payment' && (
